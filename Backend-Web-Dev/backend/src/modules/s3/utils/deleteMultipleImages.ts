...
export const deleteMultipleImages = async (link: string[]) => {
  ...
  try {
    ...
    console.log(
      "✅ " +
        ...
        ` - ☁️  Delete multiple image s3: ${keys.length} images`
    );

    return response;
  } catch (error) {
    logError("deleteMultipleImages", "Delete image error", error);
    //throw error;
  }
};
