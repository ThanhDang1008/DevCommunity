...
export class UploadVideoS3 {
  public async create(req: Request, res: Response, next: NextFunction) {
    ...
    const message =
      req.body.message || "Tải video thất bại! Vui lòng thử lại sau";
    if (!req.file) {
      return res.status(400).json({
        statusCode: 400,
        message: message,
        status: "UPLOAD_VIDEO_ERROR",
      });
    }
    ...
    console.log(
      "✅ " +
        ...
        ` - Upload video: ${req?.file?.filename} | ${req?.file?.size} bytes`
    );
    ...
    try {
      ...
      const params: PutObjectCommandInput = {
        ...
        Key: `video/${req?.file?.filename}`,
        Body: fileBuffer,
        ContentType: req?.file?.mimetype,
        ACL: ObjectCannedACL.public_read, // Cho phép file có thể được truy cập công khai
      };

      const fileUrl = `${config.CLOUDFLARE_R2_DOMAIN}/${config.CLOUDFLARE_R2_BUCKET_NAME}/video/${req?.file?.filename}`;
      ...
      logError("upload", "Upload video error", error);
      return next(new ServerError("Tải hình thất bại!", "UPLOAD_VIDEO_ERROR"));
    }
  }
}
