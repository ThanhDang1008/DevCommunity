...
import { createChildTag, checkExistChildTag } from "@/service/api/tag";
import { queryKeys } from "@/constants/Common";
import type { CreateChildTag } from "@/service/api/tag/types";
...
const ModalCreateChildTag = (props: ModalCreateChildTagProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await checkExistChildTag(
          props.tag_id,
          debouncedSlugTag
        );
  ...
  const { mutate: createChildTagMutation, isPending } = useMutation({
    mutationFn: (data: CreateChildTag) => createChildTag(data),
    onSuccess: (data) => {
...
export default ModalCreateChildTag;
