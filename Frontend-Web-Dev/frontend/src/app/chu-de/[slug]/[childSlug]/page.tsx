...
import type { ITag, ChildTag } from "@/service/api/tag/types";
import { URL } from "@/constants/Common";

import Container from "@/components/layout/home/container";
import Footer from "@/components/layout/footer";
import Header from "@components/layout/header";
import PaginationChildCategory from "./pagination";
...
export async function generateMetadata({
  ...
  const response = await getDetailTagBySlug([
    `${(await params).slug}`,
    `${(await params).childSlug}`,
  ]);
  const dataTag = response?.data?.data as DataTag;
  ...
  return {
    title: dataTag[1]?.name,
    description:
      dataTag[1]?.description ||
      `Xem các bài viết mới nhất về ${dataTag[1]?.name} từ ${metadataGlobal.applicationName}`,
    keywords: [
      `${dataTag[0]?.name}`,
      `${dataTag[1]?.name}`,
      ...metadataGlobal.keywords,
    ],
    ...
    openGraph: {
      ...
      url: `${URL}/chu-de/${dataTag[0]?.slug}/${dataTag[1]?.slug}`,
    ...
    alternates: {
      canonical: `${URL}/chu-de/${dataTag[0]?.slug}/${dataTag[1]?.slug}`,
      languages: {
        vi: `${URL}/chu-de/${dataTag[0]?.slug}/${dataTag[1]?.slug}`,
      },
    },
    robots: {
      index: true,
...
export default async function Page({ params, searchParams }: Props) {
  const PAGE_POST_CHILD_CATEGORIES = Number((await searchParams)?.p) || 1;
  const LIMIT_POST_CHILD_CATEGORIES = 20;

  let dataPost = null;
  let dataTag: DataTag | null = null;
  const slug = (await params).slug;
  const childSlug = (await params).childSlug;
  ...
  try {
    const response = await getAllPostByCategoryPublic(
      PAGE_POST_CHILD_CATEGORIES,
      LIMIT_POST_CHILD_CATEGORIES,
      [`${slug}`, `${childSlug}`]
    );
    if (response.status === 200) {
  ...
  try {
    const response = await getDetailTagBySlug([`${slug}`, `${childSlug}`]);
    if (response.status === 200) {
      dataTag = response?.data?.data as DataTag;
    }
  } catch (error) {}
      ...
      <Container>
        ...
        {dataTag && dataTag.length > 0 && (
          <nav
            className={clsx(
              "border-b mb-7",
              "border-purple-300/70 dark:border-purple-800/70"
            )}
          >
                ...
                <li key={`${dataTag[0]?.slug}`} className="flex items-center">
                  <Link
                    ...
                    className={clsx(
                      ...
                      "text-gray-700 dark:text-gray-300 font-semibold text-3xl"
                    )}
                  >
                    {dataTag[0]?.name}
                  </Link>
                      ...
                      <li key={index} className="flex items-center">
                        <Link
                          ...
                          className={clsx(
                            ...
                            {
                              "border-b-2 font-medium": item.slug === childSlug,
                              "border-purple-600 text-purple-700 dark:border-purple-500 dark:text-white":
                                item.slug === childSlug,
                            }
                          )}
                        >
        ...
        {dataPost && dataPost.length > 0 && (
          <section className="mx-auto">
              ...
              {dataPost.map((data, index) => (
                <div
                  ...
                  className={clsx(
                    ...
                    "border-purple-200/40 bg-purple-50/80 hover:bg-purple-100/80",
                    "dark:border-purple-900/40 dark:bg-purple-900/20 dark:hover:bg-purple-900/30"
                  )}
                >
                  {/* Image container */}
            ...
            <div className="flex justify-center align-items-center mt-6 pb-7">
              <PaginationChildCategory
                slug={slug}
                childSlug={childSlug}
                totalPosts={totalPosts}
                limitPostChildCategory={LIMIT_POST_CHILD_CATEGORIES}
                pagePostChildCategory={PAGE_POST_CHILD_CATEGORIES}
