...
//     return {
//       spliced: Math.abs(left - xRef.current) < 5,// The distance between the slider and the target position is less than 5 pixels
...
const Verify = ({
  ...
  const verify = () => {
    ...
    const left = parseInt(blockRef.current.style.left);
    return {
      spliced: Math.abs(left - xRef.current) < 5,
      verified: stddev !== 0,
      left,
      destX: xRef.current,
    };
  ...
  // Tailwind classes based on slider state
  const getSliderContainerClasses = () => {
    const baseClasses =
      "relative text-center h-10 leading-10 mt-4 bg-gray-50 text-gray-600 border border-gray-300";
    return baseClasses;
  };

  const getSliderClasses = () => {
    const baseClasses =
      "absolute top-0 left-0 w-10 h-10 bg-white shadow-md transition-colors duration-200 cursor-grab active:cursor-grabbing hover:bg-blue-500";

    switch (sliderState) {
      case "active":
        return `${baseClasses} h-[38px] -top-px border border-blue-500`;
      case "success":
        return `${baseClasses} h-[38px] -top-px border border-green-500 !bg-green-600`;
      case "fail":
        return `${baseClasses} h-[38px] -top-px border border-red-400 !bg-red-400`;
      default:
        return baseClasses;
    }
  };

  const getSliderMaskClasses = () => {
    const baseClasses =
      "absolute left-0 top-0 h-10 border-0 border-solid border-blue-500 bg-blue-100";

    switch (sliderState) {
      case "active":
        return `${baseClasses} h-[38px] border`;
      case "success":
        return `${baseClasses} h-[38px] border border-green-500 bg-green-100`;
      case "fail":
        return `${baseClasses} h-[38px] border border-red-400 bg-red-100`;
      default:
        return baseClasses;
    }
  };

  const getSliderIconClasses = () => {
    const baseClasses = "text-lg text-black";

    switch (sliderState) {
      case "fail":
        return `${baseClasses} top-[14px]`;
      default:
        return baseClasses;
    }
  };

  const shouldShowText = sliderState === "default";
  ...
  return (
    ...
    >
      ...
      >
        {/* Slider Mask */}
        <div
          className={getSliderMaskClasses()}
        ...
        >
          ...
          <div
            ...
            onTouchStart={handleDragStart}
          >
            <div className={getSliderIconClasses() + " hover:text-white"}>
              &rarr;
            </div>
          </div>
        </div>
      ...
      {/* Refresh Icon */}
      <div
        className="absolute right-1 top-1 w-8 h-8 cursor-pointer bg-cover bg-center"
        onClick={handleRefresh}
        style={{
      ...
      {isLoading && (
        <div
          className="absolute left-0 top-0 flex flex-col justify-center items-center text-sm text-gray-600 z-10 bg-gray-200"
          style={{
            width: width + "px",
            height: height + "px",
          }}
        >
          <div
            className="w-8 h-8 mb-2 bg-cover animate-spin"
            style={{
              backgroundImage: `url(${refreshIcon})`,
              backgroundSize: "32px",
